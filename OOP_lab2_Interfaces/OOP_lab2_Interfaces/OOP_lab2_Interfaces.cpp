// OOP_lab2_Interfaces.cpp: определяет точку входа для консольного приложения.
//
...
int main()
{
	...
	array[0] = ld;
	array[1] = q;
	for (Container *c : array) //foreach for C++
	{
		PushPopContainer *ppc = dynamic_cast<PushPopContainer*>(c);
		...
		//push for deque
		//output everything
		// do the same for deque with using (while Container not isEmpty()
	}
	for (Container *c : array)
	{
		cout << "After push " << c->toString() << endl;
	}
	for (Container *c : array) //foreach for C++
	{
		PushPopContainer *ppc = dynamic_cast<PushPopContainer*>(c);
		if (ppc != NULL)
		{
			while (ppc->isEmpty() == false)
			{
				cout << "Poped " << ppc->pop() << endl;
			}
		}
	}
	for (Container *c : array)
	...
	for (Container *c : array)
	{
		...
		if (d != NULL)
		{
			...
			{
				cout << "Poped " << d->popFront() << endl;
			}
		}
	}
	for (Container *c : array)
	{
		cout << "After Pop " << c->toString() << endl;
	}
